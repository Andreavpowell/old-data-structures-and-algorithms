...
const addValues = (arr) => {
  return arr.reduce((inputNumbers,value) => inputNumbers + value, 0);
};
...
const addPurchases = (arr) => {
  return arr.reduce((totalAmount, purchasePrice) => {
    return totalAmount + purchasePrice.purchasePrice;
  },0);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce(numOfElements => numOfElements + 1);
};
...
const returnNames = (arr) => {
  let newArr = [];
  return arr.reduce((name,value) => {
    newArr.push(name = value.name);
    return newArr;
  },[]);
};
...
const reversedString = (str) => {return str.split('').reduce((last, next) => (next + last), ''); };
